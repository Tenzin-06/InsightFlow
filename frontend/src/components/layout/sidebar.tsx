...
export function Sidebar() {
  return (
    <aside className="flex h-full w-64 flex-col bg-bg-secondary">
      ...
      <nav className="flex-1 space-y-0.5 border-r border-border-default px-3 py-4" aria-label="Main navigation">
        {mainNavRoutes.map((route) => (
          <SidebarItem key={route.path} {...route} />
        ))}
      </nav>

      <div className="border-r border-border-default border-t border-border-default px-3 py-4">
